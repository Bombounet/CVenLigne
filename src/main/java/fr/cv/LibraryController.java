 ...
 /* @PostMapping("/formations/edit/{id}")
  ...
  public RedirectView editFormations(@ModelAttribute User newData, @PathVariable String id, RedirectAttributes attrs) {
    ...
    public RedirectView majFormation(@ModelAttribute Formation newData, @PathVariable(value = "id") long idlong, RedirectAttributes attrs) {
        ...
        int index = indexFormation(formations, idlong);
        formations.get(index).setName(newData.getName());
        formations.get(index).setPlace(newData.getPlace());
        formations.get(index).setYear(newData.getYear());
        formations.get(index).setDescription(newData.getDescription());
        User user = getUser(1);
    ...
    public RedirectView majExperience(@ModelAttribute Formation newData,
        ...
        int index = indexExperience(experiences, idlong);
        experiences.get(index).setName(newData.getName());
        experiences.get(index).setPlace(newData.getPlace());
        experiences.get(index).setYear(newData.getYear());
        experiences.get(index).setDescription(newData.getDescription());
        User user = getUser(1);
    ...
    public RedirectView majCompetence(@ModelAttribute Competence newData,
        ...
        competences.get(indexCompetence(competences, idlong)).setName(newData.getName());
        User user = getUser(1);
    ...
    public RedirectView majProjet(@ModelAttribute Formation newData,
        ...
        int index = indexProjet(projets, idlong);
        projets.get(index).setName(newData.getName());
        projets.get(index).setPlace(newData.getPlace());
        projets.get(index).setYear(newData.getYear());
        projets.get(index).setDescription(newData.getDescription());
        User user = getUser(1);
    ...
    public static Formation getFormation(long Id, long ID) {
        ...
        Formation formation = formations.get(indexFormation(formations, ID));
        return formation;
    }

    public static Experience getExperience(long Id, long ID) {
        List<Experience> experiences = getExperiences(Id);
        Experience experience = experiences.get(indexExperience(experiences, ID));
        return experience;
    }

    public static Projet getProjet(long Id, long ID) {
        List<Projet> projets = getProjets(Id);
        Projet projet = projets.get(indexProjet(projets, ID));
        return projet;
    }
    ...
    public static Competence getCompetence(long Id, long ID) {
        ...
        Competence competence = competences.get(indexCompetence(competences, ID));
        return competence;
    }



  public static int indexCompetence(List<Competence> competences, long ID) {
    for (int i = 0; i < competences.size(); i++) {
      System.out.println(competences.get(i).getName() + "   " + i);
      System.out.println(competences.get(i).getId() + "   " + ID);
      if (competences.get(i).getId() == ID) {
        System.out.println();
        return i;
      }
    }
  ...
  public static int indexProjet(List<Projet> projets, long ID) {
    for (int i = 0; i < projets.size(); i++) {
      if (projets.get(i).getId() == ID) {
        return i;
      }
    }
    return 0;
  ...
  public static int indexFormation(List<Formation> formations, long ID) {
    for (int i = 0; i < formations.size(); i++) {
      if (formations.get(i).getId() == ID) {
        return i;
      }
    }
    return 0;
  ...
  public static int indexExperience(List<Experience> experiences, long ID) {
    for (int i = 0; i < experiences.size(); i++) {
      if (experiences.get(i).getId() == ID) {
        return i;
      }
    }
    return 0;
